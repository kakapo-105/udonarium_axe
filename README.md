...
このリポジトリは [Xelltis/udonarium_axe](https://github.com/Xelltis/udonarium_axe) の個人フォークです。
次の機能は本家の Udonarium Axe にはなく、このフォークで独自に加えたカスタマイズです。

| 機能                          | 内容                                                                                                                                                                                                                                                                                                                                                                                | 説明                                                                     |
| ----------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| ターゲット数の参照 `{tcount}` | ターゲットを取る行で、選んだターゲットの数に置き換わる。`t:HP-10 :MP-3*{tcount}` のように対象数に応じた消費を書ける                                                                                                                                                                                                                                                                 | [チャットの特殊記法](website/manual/chat-syntax.md#ターゲットの数を読む) |
| チャットパレットの 2 段見出し | `◆` の見出しの下に `■` の見出しを置ける。見出しのボタンの一覧では `◆` の横に `■` がサブメニューで開く                                                                                                                                                                                                                                                                               | [チャットパレット](website/manual/chat-palette.md#見出しを-2-段にする)   |
| 条件分岐 `if(条件,真,偽)`     | 値や `{tcount}`、リソース操作では `[2d6]` の出目で数値を切り替える。入れ子と `$1` での出目の使い回しに対応。計算項目の式でも使える                                                                                                                                                                                                                                                  | [チャットの特殊記法](website/manual/chat-syntax.md#条件で値を切り替える) |
| MCP による AI 操作            | Claude Code などの AI から MCP 経由で、部屋・コマの確認、コマの移動、チャット送信、キャラクターのシートの確認、チャットパレットからの発言、バフ／デバフの確認と操作（バフマネージャーと同じ卓全体の編集を含む）ができる。[okamichi さんの feat/mcp](https://github.com/okamichi/udonarium_axe/tree/feat/mcp) を 1.57.1 向けに移植したもの。操作を許可するパネルはドラッグで動かせる | [MCP の導入手順](tools/mcp-server/README.md)                             |
| 参加者向けマニュアル          | 利用ガイドをアプリと同じサイトの `docs/` に公開し、アプリのメニューの「マニュアル」から開ける                                                                                                                                                                                                                                                                                       | [公開先](https://kakapo-105.github.io/udonarium_axe/docs/)               |
| キャラクター一覧              | 卓のキャラクターを画面の端に並べ、リソースのバーと値を表示する（ココフォリア風）。項目はポップアップ表示と同じで、キャラごとに一覧から外せる                                                                                                                                                                                                                                        | [キャラクター](website/manual/character.md#画面の端のキャラクター一覧)   |
| テーブルごとの BGM            | テーブル設定で曲を選んでおくと、そのテーブルに切り替えたときに全員へ流れる（ココフォリアのシーン BGM に相当）                                                                                                                                                                                                                                                                       | [テーブル設定](website/manual/table-setting.md#テーブルごとの-bgm)       |

フォークの運用（公開先・上流の取り込み方）は [FORK.md](FORK.md) にまとめています。
...
| [tools/mcp-server/README.md](tools/mcp-server/README.md) | MCP による AI 操作の導入と使い方   |
